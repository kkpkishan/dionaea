...
curl -s -XDELETE https://IP:9200/_template/logstash -u wazuh-user:wazuh-password --insecure
echo
echo "Checking if empty."
curl -s -XGET https://IP:9200/_template/logstash -u wazuh-user:wazuh-password --insecure
echo
echo "Putting default template."
curl -s -XPUT "https://IP:9200/_template/logstash" -u wazuh-user:wazuh-password --insecure -H 'Content-Type: application/json' -d'
{
